...
firebaseApp.analytics().logEvent('Greece');
...
const url ="http://api.abrajmaktoob.com/api-horoscope?action=greece";
...
componentWillMount() {
  if(connectionStatus)
  {
    ...
    .then((responseJson) => {
        this.setState({
            data: responseJson,
            myText: responseJson.result_greece.year.post_content,
        })
    })
    .catch((error) => {
        console.error(error);
